...
- **Resulting App State:** verified on a Pixel 9. The blob morph was measured rather than eyeballed — silhouette area swings **15.7%** (30,320 → 25,558 px) while the bounding box expands, which a background gradient cannot do; at `animator_duration_scale=0` three frames are byte-identical, confirming the reduced-motion path. **NFC confirmed against a real contactless card**: number and expiry captured, the expiry stored in `YYYY-MM` exactly as the `MMYY` → `YYYY-MM` conversion predicts. Cardholder name absent — that issuer does not expose tag `5F20`, which is the documented "sometimes available" case now confirmed in the field rather than in theory.
