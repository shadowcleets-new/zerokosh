package org.bharatvault.core.crypto
 ...
 * raw bytes mod 32), grouped as BVR-XXXXX-XXXXX-XXXXX-XXXXX-XXXXX-XX.
 */
object RecoveryKey {
    ...
    fun encode(bytes: ByteArray): String {
        ...
        return "BVR-" + sb.chunked(5).joinToString("-")
    }

    /** Case-insensitive; strips BVR-/hyphens/spaces; maps I→1, L→1, O→0. Null when malformed or checksum fails. */
    fun decode(input: String): ByteArray? {
        var s = input.trim().uppercase()
            .replace("-", "").replace(" ", "")
            .replace('I', '1').replace('L', '1').replace('O', '0')
        if (s.startsWith("BVR")) s = s.substring(3)
