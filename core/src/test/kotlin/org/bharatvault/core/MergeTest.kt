...
class MergeTest {
    ...
    fun `attachments union by uuid`() {
        val a = VaultBody(attachments = mapOf("att1" to org.bharatvault.core.model.Attachment("a.txt", "text/plain", "QQ==")))
        val b = VaultBody(attachments = mapOf("att2" to org.bharatvault.core.model.Attachment("b.txt", "text/plain", "Qg==")))
        val merged = VaultMerge.merge(a, b, now, ::freshUuid)
