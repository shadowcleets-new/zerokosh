...
class VaultRoundTripTest {
    ...
    fun `header params are honored by readers`() {
        ...
        val created = createFixtureFile()
        val envelope = org.bharatvault.core.vault.VaultFileCodec.decode(created.fileBytes)
        assertEquals(mem, envelope.header.mem)
        assertEquals(3, envelope.header.ops)
    }

    /**
     * §11.2 cross-platform gate: commits fixtures/vault_android.bvlt (created
     * once; subsequent runs verify it still opens with zero data loss).
     */
    @Test
    fun `fixture vault_android bvlt exists and opens losslessly`() {
        val fixture = File("../fixtures/vault_android.bvlt").canonicalFile
        if (!fixture.exists()) {
