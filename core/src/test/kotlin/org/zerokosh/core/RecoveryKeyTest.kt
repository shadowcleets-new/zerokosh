...
/** Appendix C: Crockford Base32 + checksum, format KSH-XXXXX-XXXXX-XXXXX-XXXXX-XXXXX-XX. */
class RecoveryKeyTest {

    @Test
    fun `encode format matches spec grouping`() {
        val encoded = RecoveryKey.encode(ByteArray(16))
        assertTrue(Regex("^KSH-[0-9A-Z]{5}-[0-9A-Z]{5}-[0-9A-Z]{5}-[0-9A-Z]{5}-[0-9A-Z]{5}-[0-9A-Z]{2}$").matches(encoded), encoded)
    }
    ...
    fun `malformed inputs reject`() {
        ...
        assertNull(RecoveryKey.decode("KSH-SHORT"))
        assertNull(RecoveryKey.decode("KSH-UUUUU-UUUUU-UUUUU-UUUUU-UUUUU-UU")) // U not in Crockford
    }

    @Test
    fun `checksum is sum of raw bytes mod 32`() {
        val raw = ByteArray(16) { 1 } // sum = 16 -> 'G' (index 16)
        val encoded = RecoveryKey.encode(raw).replace("KSH-", "").replace("-", "")
