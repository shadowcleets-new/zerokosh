...
Hilt/Dagger/Koin are PROHIBITED (Rules §0.3). The entire object graph is constructed once in `BharatVaultApp.onCreate()` and reached via `application as BharatVaultApp`. This is the whole DI system; do not add lazy service locators, `object` singletons holding Context, or companion factories.

## 2.2 The graph (application-scoped, constructed in this order)

```
BharatVaultApp.onCreate()
...
|---|---|---|---|
| `Prefs` | `(context: Context)` | app-singleton | `BharatVaultApp.prefs` |
| `AssetCatalog` | `(context: Context)` | app-singleton | `BharatVaultApp.catalog` |
...
| `VaultRepository` | `(crypto: AndroidCrypto, store: VaultStore, prefs: Prefs)` | app-singleton | `BharatVaultApp.repository` |
...
| `BharatVaultAutofillService` | `()` (system) | system service | reaches graph via `application as BharatVaultApp` |
| `MainActivity` | `()` (system) | activity | `private val app get() = application as BharatVaultApp` |
...
## 2.4 Composables receive the graph as a single `app: BharatVaultApp` parameter

Every screen composable's first parameter is `app: BharatVaultApp` (see the frozen screens). New screens in Phase 8 follow the identical convention. ViewModels are NOT introduced — state lives in `VaultRepository` StateFlows + `remember` (Phase 4 documents why this is the frozen architecture).
