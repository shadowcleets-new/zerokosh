# Zerokosh — Master Build Manual v2.0 — Fully-Native Edition

**What this document is:** a complete, self-contained specification that any implementer — a human developer, a large AI model, or a small AI model — can follow, section by section, to build Zerokosh as **four fully native applications: Android, iOS, Windows, and macOS**.

**v2.0 architecture lock:** Zerokosh is built with each platform's own native language and first-party toolkit — **Kotlin + Jetpack Compose (Android), Swift + SwiftUI (iOS and macOS), C# + WinUI 3 (Windows)**. NO cross-platform framework — Flutter, React Native, Kotlin Multiplatform, Compose Multiplatform, .NET MAUI, Electron, Tauri, or any WebView/shared-runtime UI — may be used anywhere in this project. Interoperability between the four apps is guaranteed by this specification and its conformance suite, **not by shared code**. Rationale and rejected alternatives: Appendix G (ADR-001).

**What Zerokosh is:** an open-source (GPL-3.0), local-first, zero-server password and credential vault designed for Indian users. It models Indian financial life correctly (bank accounts with transaction/profile passwords, ATM PINs, UPI PINs, IFSC codes, LPG IDs, FASTag, demat accounts) and is free forever for individuals because it has no backend to pay for.
...
Indian users distrust cloud password managers with banking credentials, are highly price-sensitive, and live in an OTP-first app ecosystem (Zepto/Blinkit/Swiggy/Zomato log in with phone + OTP, so there is often no password — the vault stores the *identity around* those apps: registered phone, wallet PINs, gift cards, memberships, TOTP, passkeys). One Indian bank relationship produces 8–12 distinct secrets that global apps cannot model. Zerokosh wins by (a) modelling India correctly via templates, (b) being structurally unable to see user data (local-first, E2EE, no accounts), and (c) being free because there is nothing to host. The user's chosen **sync folder** (which may live inside Google Drive, iCloud Drive, OneDrive, or any synced directory) carries the encrypted vault between devices; Zerokosh itself never talks to any cloud.
...
RecoveryKey = 16 random bytes, shown once as KSH-XXXXX-XXXXX-XXXXX-XXXXX-XXXXX-XX
...
## §4. VAULT FILE FORMAT v1 (`.kosh` — byte-identical across platforms)
...
**§4.4 Atomic write (MANDATORY, prevents corruption):** serialize → encrypt → write to `vault.kosh.tmp` in same directory → flush/fsync → verify by re-reading and AEAD-opening the tmp file → rename over `vault.kosh`. Before the first write of a session, copy current file to `vault.kosh.bak`. NEVER write the vault in place.
...
3. Cloud sync clients may briefly present two files (`vault.kosh` and a "conflicted copy"); on unlock, if any sibling file matching `vault*.kosh` exists with the same `vault_uuid`, merge it in and delete it.
...
| S4 | Recovery Kit | Shows RecoveryKey once, formatted `KSH-…`; buttons: "Save as PDF" (A4, key as text+QR, instructions "keep with your property papers", localized) and "I have written it down" (must check a confirm box). NEVER show this key again after this screen; Settings offers "Generate NEW recovery key" (re-wraps `wrap_rk`) |
...
### §5.6 Sync folder
Settings → "Choose sync folder": OS folder picker; persist durable access (SAF persisted permission / security-scoped bookmark / saved path). The app reads/writes ONLY `vault.kosh`, `vault.kosh.tmp`, `vault.kosh.bak`, and conflict siblings inside that folder. If no folder chosen, vault lives in app-private storage and Settings shows a "Not backed up" badge. First-run S5 footer offers "Set up backup folder now".

### §5.7 Reminders
Local notifications only. For any DATE field with a reminder (`expiry`, `premium_due_date`, `membership_renewal`, `renewal_date` auto-suggest one at 30 days), schedule via the platform scheduler. Notification text NEVER contains secret values — format: "SBI credit card expires this month · open Zerokosh".
...
**§6.3 Project setup:** Android Studio → New Project → "Empty Activity (Compose)" → package `org.zerokosh.app` → add `:core` module (Kotlin JVM library) → copy `/spec/templates.json`, `pickers.json`, `bank_rules.json` into `app/src/main/assets/`.
...
**§7.1 Stack (locked):** Swift 5.10 · SwiftUI · iOS 16+ · Xcode 15+. Targets: `Zerokosh` (app), `ZerokoshCore` (Swift package: model/crypto/vault/merge, NO UIKit — an independent Swift implementation of §2–§4, shared ONLY with the macOS target per §9.1, which is native-within-Apple and explicitly permitted by R0.2 item 8; never shared with Android or Windows), `AutoFillExtension` (Credential Provider). Dependencies (complete list): `swift-sodium` (SPM), `CodeScanner` or native `AVFoundation`+`Vision` for QR (prefer native = zero dep). Serialization via `Codable` (built-in).
...
```
Run §3.6 vectors in `ZerokoshCoreTests` (M1).
...
**§8.1 Stack (locked):** C# / .NET 8 · **WinUI 3** (Windows App SDK, packaged MSIX) · Windows 10 1809+. Projects: `Zerokosh.Core` (net8 class library: model/crypto/vault/merge — no UI refs; an **independent C# implementation** of §2–§4, NOT a port, binding, or transpilation of the Kotlin or Swift cores; runs §3.6/§11.2 tests via xUnit) and `Zerokosh.App` (WinUI 3). Dependencies (complete list): `Geralt` (libsodium binding — Argon2id + XChaCha20-Poly1305), `Microsoft.WindowsAppSDK`, `CommunityToolkit.Mvvm`, `System.Text.Json` (built-in). QR scan not needed on desktop (TOTP added by pasting the secret/URI).
...
**§8.4 Quick unlock (Windows Hello):** gate = `UserConsentVerifier.RequestVerificationAsync("Unlock Zerokosh")`; storage = MasterKey wrapped with DPAPI `ProtectedData.Protect(masterKey, entropy, DataProtectionScope.CurrentUser)` where `entropy` = 32 random bytes kept in the app's local settings. Honest limitation (document in Security page): DPAPI binds to the Windows user account, not to the biometric itself; the passphrase remains the root secret. Prefer `KeyCredentialManager` (Hello-backed key signs a challenge that becomes the wrap entropy) when available — implement if straightforward, else record in DECISIONS.md.
...
16 random bytes → Crockford Base32 (no padding) → 26 chars → append 1 checksum char = Crockford digit of (sum of the 16 raw bytes mod 32) → group as `KSH-XXXXX-XXXXX-XXXXX-XXXXX-XXXXX-XX`. Decoder strips `KSH-` and hyphens, is case-insensitive, maps I→1, L→1, O→0, verifies checksum before attempting §3.3 unwrap.
...
**Context:** Zerokosh must ship on Android, iOS, Windows, and macOS as a security product whose value depends on hardware-backed key storage and biometrics, deep OS credential/autofill integration, small auditable binaries, and user trust.
...
*End of manual v2.0. A model that has completed §0–§5 plus one platform section has everything required to produce a conformant, interoperable, fully native Zerokosh application. The three core implementations are kept honest not by sharing code, but by sharing this specification.*
